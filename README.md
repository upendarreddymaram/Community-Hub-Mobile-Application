...
- **Communities & posts** are loaded live from the public [Discourse Meta forum API](https://meta.discourse.org/site.json) (`meta.discourse.org`).
- **Join/leave** and **create post** are stored locally on-device (Discourse requires authenticated API keys for writes).
...
4. **Mutations**: Join/leave and create-post use optimistic updates with rollback on failure and retry UI.
5. **Offline**: React Query persister caches successful responses; join/leave actions enqueue when offline and sync via `useOfflineSync` when connectivity returns.
...
- **Write queue**: Join/leave actions stored in Zustand offline queue when offline
- **Sync on reconnect**: Queue processed sequentially; caches invalidated on success
...
| API | Discourse Meta public API (reads) + local persistence (writes) | Real network data; join/post not synced to Discourse |
| Navigation | React Navigation native stack | Simple stack fits scope; tab navigation omitted intentionally |
| List performance | FlatList with memoized `CommunityCard`, tuned render batch | Good for ~45 live categories; FlashList at larger scale |
| Error handling | ErrorBoundary + per-screen ErrorView | Boundary catches render errors; query errors handled inline with retry |
| Secure token | AsyncStorage session | Mock auth token persisted with session JSON for simplicity |
...
- [x] Offline detection, cached data, queued membership actions
- [x] Loading, empty, and error states throughout
- [x] ESLint + Prettier configured
- [x] Error boundary (bonus)
- [x] Accessibility labels on interactive elements (bonus)
...
- **Feature-based modules**: Co-locate screens, hooks, and components per domain
- **Dark mode**: Theme context with system preference
- **FlashList**: Replace FlatList for larger datasets
